  ...
  return (
    <div className="flex-1 flex flex-col h-screen overflow-y-auto bg-surface p-container space-y-6 animate-fade-in">
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        <div className="glass-card p-4 bg-slate-900 border border-slate-800 rounded-xl flex items-center justify-between text-white">
          <div className="space-y-1">
            <span className="label-caps text-slate-400 text-[10px] tracking-wider block">Total Logged Events</span>
        ...
        <div className="glass-card p-4 bg-red-950/40 border border-red-900/60 rounded-xl flex items-center justify-between text-white">
          <div className="space-y-1">
            <span className="label-caps text-red-300 text-[10px] tracking-wider block">Active Critical Alarms</span>
            <span className="text-2xl font-bold font-mono text-critical">{stats.critical}</span>
          </div>
          <div className="p-2 rounded bg-critical/20 text-critical animate-pulse">
            <AlertTriangle size={20} />
          </div>
        </div>

        <div className="glass-card p-4 bg-amber-950/30 border border-amber-900/40 rounded-xl flex items-center justify-between text-white">
          <div className="space-y-1">
            <span className="label-caps text-amber-300 text-[10px] tracking-wider block">Active Warnings</span>
            <span className="text-2xl font-bold font-mono text-warning">{stats.warning}</span>
          </div>
        ...
        <div className="glass-card p-4 bg-slate-50 border border-border rounded-xl flex items-center justify-between text-on-surface">
          <div className="space-y-1">
            <span className="label-caps text-on-surface-variant text-[10px] tracking-wider block">Pending Acks</span>
            <span className="text-2xl font-bold font-mono text-primary">{stats.active}</span>
          </div>
      ...
      <div className="glass-card bg-surface-container-lowest border border-border rounded-xl p-5 flex flex-col space-y-4">
        {/* Filtering Options */}
        <div className="flex flex-col md:flex-row justify-between gap-4 border-b border-border-light pb-4">
            ...
            <button
              ...
              className={`px-3 py-1.5 rounded-lg text-xs font-semibold transition-all ${
                filterSeverity === 'warning'
                  ? 'bg-warning/10 text-[#92400E] border border-warning/20'
                  : 'text-on-surface-variant hover:bg-slate-100'
              }`}
            >
              Warning
          ...
          <div className="relative">
            ...
            <input
              ...
              className="pl-9 pr-4 py-2 text-xs w-[260px] bg-white border border-border rounded-lg focus:outline-none focus:border-primary shadow-sm"
            />
          </div>
        </div>
            ...
            filteredAlerts.map((alert) => (
              <div
                ...
                className={`p-4 rounded-xl border flex items-start justify-between gap-4 text-xs transition-all ${
                  alert.acknowledged
                    ? 'bg-slate-50/50 border-border-light opacity-75'
              ...
              >
                <div className="flex gap-3">
                  ...
                  <div className="space-y-1">
                    <div className="flex items-center gap-2.5 flex-wrap">
                      ...
                      {alert.acknowledged && (
                        <span className="text-[10px] text-success bg-success-light px-2 py-0.5 rounded-full font-bold">
                          ACKNOWLEDGED
                        </span>
                      )}
                    </div>
                    ...
                    <div className="flex items-center gap-3 text-[10px] text-on-surface-variant mt-2 font-mono flex-wrap">
                      ...
                      {alert.transition !== 'N/A' && (
                        <>
                          <span>•</span>
                          <span>Transition: <strong className="text-on-surface-variant font-mono bg-slate-100 px-1 py-0.2 rounded">{alert.transition}</strong></span>
                        </>
                      )}
                      <span>•</span>
                ...
                {!alert.acknowledged && (
                  <button
                    ...
                    className="flex items-center gap-1.5 px-3 py-1.5 bg-white hover:bg-slate-50 border border-border text-[11px] font-bold text-on-surface-variant hover:text-success rounded-lg shadow-sm transition-all"
                  >
                    <CheckCheck size={14} />
                    Acknowledge
                  </button>
